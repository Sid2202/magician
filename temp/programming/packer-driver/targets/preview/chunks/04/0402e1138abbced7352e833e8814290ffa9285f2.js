    ...
    execute: function () {
      ...
      _export("SpawnSystem", SpawnSystem = class SpawnSystem {
        ...
        scrollActiveObjects(dx) {
          ...
          for (var npc of this.activeNPCs) {
            npc.scrollX(dx);
        ...
        _spawnItem(screenHalfH) {
          ...
          var node = this._pooling.getFromPool(poolType);

          if (!node) return;
          node.setParent(this._spawnParent);
          var ctrl = node.getComponent(_crd && ItemController === void 0 ? (_reportPossibleCrUseOfItemController({
            error: Error()
          }), ItemController) : ItemController);
          if (!ctrl) return;
          var y = (_crd && MathUtil === void 0 ? (_reportPossibleCrUseOfMathUtil({
            error: Error()
        ...
        _spawnNPC(screenHalfH) {
          var node = this._pooling.getFromPool((_crd && PoolType === void 0 ? (_reportPossibleCrUseOfPoolType({
            error: Error()
          }), PoolType) : PoolType).NPC);

          if (!node) return;
          node.setParent(this._spawnParent);
          var ctrl = node.getComponent(_crd && NPCController === void 0 ? (_reportPossibleCrUseOfNPCController({
            error: Error()
          }), NPCController) : NPCController);
          if (!ctrl) return;
          var idx = Math.floor(Math.random() * NPC_WANTS.length);
          var wantsItem = NPC_WANTS[idx];
          var reward = NPC_REWARDS[idx];
          var y = (_crd && MathUtil === void 0 ? (_reportPossibleCrUseOfMathUtil({
            error: Error()
        ...
        _recycleItem(ctrl) {
          var idx = this.activeItems.indexOf(ctrl);
          if (idx !== -1) this.activeItems.splice(idx, 1);
          ctrl.deactivate();
          var model = ctrl.getModel();
          var poolType = model.type;

          this._pooling.returnToPool(poolType, ctrl.node);
        }

        _recycleNPC(ctrl) {
          var idx = this.activeNPCs.indexOf(ctrl);
